...
```
sp_cs -e HashID ActorName Field "path\to\update/dlc(0010)\content" "path\to\out\File or Folder" [path\to\update\content] (only if the first path\to\update/dlc is dlc)
```
